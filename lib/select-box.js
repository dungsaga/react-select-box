...
module.exports = React.createClass({displayName: 'exports',
  ...
  onTouchStart: function (e) {
    ...
    setTimeout(this.closeOnOutsideTouched.bind(this, e), 100)
  },

  closeOnOutsideTouched: function (e) {
    if (!this.state.open) { return }
    if (this.isOutsideTouched(this.refs.menu, e)) {
      this.handleClose()
    }
  },

  isOutsideTouched: function (domNode, e) {
    var rect = domNode && domNode.getBoundingClientRect()
  ...
  renderOption: function (option, i) {
    ...
    var doNothing = function(event) { event.preventDefault() }
    return a(
      {
        id: this.state.id + '-' + i,
        href: '#',
        onClick: option.disabled ? doNothing : this.handleChange(option.value),
